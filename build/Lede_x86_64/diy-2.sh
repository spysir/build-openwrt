...
sed -i '/uci commit system/i\uci set system.@system[0].hostname='VMR'' package/lean/default-settings/files/zzz-default-settings
...
sed -i "s/OpenWrt /XPK compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" package/lean/default-settings/files/zzz-default-settings
# 修改opentopd为默认主题,可根据你喜欢的修改成其他的（不选择那些会自动改变为默认主题的主题才有效果）
